...
# TASK 4d CHANGE start
def GetCellReference(Sim):
    print()

    # Validate Row
    while True:
        RowInput = input("Enter row number: ")
        try:
            Row = int(RowInput)
            if Row < 1 or Row > Sim._NumberOfRows:
                print(f"Error: Row must be between 1 and {Sim._NumberOfRows}.")
                continue
            break
        except ValueError:
            print("Error: Row must be a whole number.")

    # Validate Column
    while True:
        ColInput = input("Enter column number: ")
        try:
            Column = int(ColInput)
            if Column < 1 or Column > Sim._NumberOfColumns:
                print(f"Error: Column must be between 1 and {Sim._NumberOfColumns}.")
                continue
            break
        except ValueError:
            print("Error: Column must be a whole number.")
