...
class Simulation():
    ...
    def RelocateAnt(self):
        print("\nSelect the cell containing the worker ant to move:")
        fromRow, fromColumn = GetCellReference()

        workerToMove = None
        for i in self._Ants:
            if isinstance(i, WorkerAnt) and i.GetRow() == fromRow and i.GetColumn() == fromColumn:
                workerToMove = i
                break

        if workerToMove is None:
            print("No worker ant found at that location.\n")
            return

        print("Select the destination cell:")
        toRow, toColumn = GetCellReference()

        workerToMove.SetNewLocation(toRow, toColumn)
        print(f"Worker ant {workerToMove.GetID()} moved to Row {toRow}, Column {toColumn}.\n")
    # TASK 8 CHANGE end
...
class WorkerAnt(Ant):
    ...
    # TASK 8 CHANGE start
    def SetNewLocation(self, newRow, newColumn):
        self._Row = newRow
