...
class Simulation():
    ...
    def SetUpANestAt(self, Row, Column):
        ...
        for Worker in range(2, self._StartingAntsInNest + 1):
            ...
            # TASK 10 CHANGE start
            self._Ants[-1]._SimulationRef = self
            # TASK 10 CHANGE end
        ...
        for Row in range(1, self._NumberOfRows + 1):
            for Column in range(1, self._NumberOfColumns + 1):
                ...
                if self._Weather is not None:
                    if self._Weather.GetRow() == Row and self._Weather.GetColumn() == Column:
                        Details += "| STORM PRESENT |  "
                # TASK 10 CHANGE end
                if AmountOfFood > 0:
        ...
        for Count in range(1, NumberOfStages + 1):
            ...
            if self._Weather is None:
                if random.randint(1, 100) <= 75:
                    stormRow = random.randint(1, self._NumberOfRows)
                    stormColumn = random.randint(1, self._NumberOfColumns)
                    self._Weather = Weather(stormRow, stormColumn)
                    print(f"Storm begins at cell {stormRow}, {stormColumn}")
            else:
                self._Weather = self._Weather.AdvanceStage(self._Nests, self._Ants, self._Pheromones)
                if self._Weather is None:
            ...
            for A in self._Ants:
                ...
                if A.GetFoodCarried() > 0 and A.IsAtOwnNest():
                    # TASK 10 CHANGE start
                    if self._Weather is not None and A.GetRow() == self._Weather.GetRow() and A.GetColumn() == self._Weather.GetColumn():
                        print("An ant was killed returning to a storm-hit nest.")
                        self._Ants.remove(A)
                        continue
                    # TASK 10 CHANGE end
        ...
        if self._AmountOfFoodCarried > 0:
            ...
            # TASK 10 CHANGE start
                ...
                while not chosen:
                    ...
                    newRow, newColumn = self._ChangeCell(indexToUse, self._Row, self._Column)

                    if hasattr(self, "_SimulationRef"):
                        w = self._SimulationRef._Weather
                        if w is not None and w.GetRow() == newRow and w.GetColumn() == newColumn:
                            continue
                    chosen = True

                self._Row, self._Column = newRow, newColumn
            # TASK 10 CHANGE end
        else:
...
class Weather(Entity):
    ...
    def AdvanceStage(self, nests, ants, pheromones):
        antsDestroyed = 0
        pheromonesDestroyed = 0

        for a in ants[:]:
            if a.GetRow() == self._Row and a.GetColumn() == self._Column:
                ants.remove(a)
                antsDestroyed += 1

        for p in pheromones[:]:
            if p.GetRow() == self._Row and p.GetColumn() == self._Column:
                pheromones.remove(p)
                pheromonesDestroyed += 1

        self._Duration -= 1
        print(f"Storm destroyed {antsDestroyed} ants and {pheromonesDestroyed} pheromones.")
