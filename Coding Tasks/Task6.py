...
class Simulation():
    ...
    def AdvanceStage(self, NumberOfStages):
        for Count in range(1, NumberOfStages + 1):
            ...
            for A in self._Ants:
                ...
                if A.GetTypeOfAnt() == "flying":
                    if A.GetStages() >= 3:
                        ...
                        NewRow = 0
                        NewColumn = 0
                        while not NestCreated:
                            NewRow = random.randint(1, self._NumberOfRows)
                            NewColumn = random.randint(1, self._NumberOfColumns)
                            LocationValid = True
                            for N in self._Nests:
                                if N.GetRow() == NewRow and N.GetColumn() == NewColumn:
                                    LocationValid = False
                            if LocationValid:
                                self.SetUpANestAt(NewRow, NewColumn)
                                print(f"Flying ant has flown to {NewRow}, {NewColumn} and set up a new nest.")
                                NestCreated = True
                        AntsToDelete.append(A)
                        continue
                # TASK 6 CHANGE end
            ...
            # TASK 6 CHANGE start
            for A in AntsToDelete:
                self._Ants.remove(A)
            
            OldAntCount = len(self._Ants)
            # TASK 6 CHANGE end
            ...
            # TASK 6 CHANGE start
            NewAntCount = len(self._Ants)
            if NewAntCount > OldAntCount:
                for i in range(OldAntCount, NewAntCount):
                    if self._Ants[i].GetTypeOfAnt() == "flying":
