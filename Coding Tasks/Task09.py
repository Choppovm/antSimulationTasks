...
class Simulation():
    def __init__(self, SimulationParameters):
        ...
        # TASK 9 CHANGE start
        self._Bird = Bird(
            random.randint(1, self._NumberOfRows),
            random.randint(1, self._NumberOfColumns)
        )
        # TASK 9 CHANGE end
    ...
    def AdvanceStage(self, NumberOfStages):
        for Count in range(1, NumberOfStages + 1):
            ...
            # TASK 9 CHANGE start
            neighbourIndices = self.__GetIndicesOfNeighbours(self._Bird.GetRow(), self._Bird.GetColumn())
            self._Bird.MoveRandomly(neighbourIndices)

            birdCell = self._Grid[self.__GetIndex(self._Bird.GetRow(), self._Bird.GetColumn())]
            antsHere = [A for A in self._Ants if A.InSameLocation(birdCell)]
            antsEaten = 0

            if len(antsHere) > 0:
                toEat = min(5, len(antsHere))
                for i in range(toEat):
                    deadAnt = antsHere[i]
                    self._Ants.remove(deadAnt)
                    antsEaten += 1

                print(f"Bird at ({self._Bird.GetRow()}, {self._Bird.GetColumn()}) ate {antsEaten} ants.")
            # TASK 9 CHANGE end
            for N in self._Nests:
...
class Bird(Entity):
    ...
    def MoveRandomly(self, listOfNeighbours):
        chosen = False
        while not chosen:
