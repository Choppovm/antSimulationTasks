...
class Simulation():
    ...
    # TASK 7 CHANGE start
    def GetIndexOfNeighbourWithStrongestSmell(self, Row, Column):
        StrongestSmell = 0
        IndexOfStrongestSmell = -1
        NeighbourIndices = self.__GetIndicesOfNeighbours(Row, Column)
        
        for Index in NeighbourIndices:
            if Index != -1:
                CellSmell = self._Grid[Index].GetFoodSmell()
                if CellSmell > StrongestSmell:
                    StrongestSmell = CellSmell
            ...
            for A in self._Ants:
                ...
                if A.GetFoodCarried() > 0 and A.IsAtOwnNest():
                    ...
                    if A.GetTypeOfAnt() == "forager":
                        if A.GetFoodCarried() > 0:
                            print("Forager ant is carrying food to nest.")
                            A.ChooseCellToMoveTo(self.__GetIndicesOfNeighbours(A.GetRow(), A.GetColumn()), -1)
                        else:
                            IndexOfStrongestSmell = self.GetIndexOfNeighbourWithStrongestSmell(A.GetRow(), A.GetColumn())
                            if IndexOfStrongestSmell != -1:
                                Neighbours = self.__GetIndicesOfNeighbours(A.GetRow(), A.GetColumn())
                                DirectionIndex = Neighbours.index(IndexOfStrongestSmell)
                                Directions = ["North-West", "North", "North-East", "West", "Center", "East", "South-West", "South", "South-East"]
                                print(f"Forager ant has detected food to the {Directions[DirectionIndex]} and is moving towards it.")
                                A.ChooseCellToMoveTo(Neighbours, IndexOfStrongestSmell)
                            else:
                                print("Forager ant is looking for food.")
                                A.ChooseCellToMoveTo(self.__GetIndicesOfNeighbours(A.GetRow(), A.GetColumn()), -1)
                    else:
...
class ForagerAnt(Ant):
    ...
    def ChooseCellToMoveTo(self, ListOfNeighbours, IndexOfTarget):
        if self._AmountOfFoodCarried > 0:
            if self._Row > self._NestRow:
                self._Row -= 1
            elif self._Row < self._NestRow:
            ...
            IndexToUse = ListOfNeighbours.index(IndexOfTarget)
            self._Row, self._Column = self._ChangeCell(IndexToUse, self._Row, self._Column)
# TASK 7 CHANGE end
