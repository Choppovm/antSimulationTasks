...
def Main():         #Q1
    ...
    endSim = False # TASK 1 CHANGE (new while loop)
    while not endSim: # TASK 1 CHANGE (new while loop)
        if SimNo == "1":
        ...
        # TASK 1 CHANGE start
        quitCheckedBool = False
        while not quitCheckedBool:
            quitCheck = input("Are you sure you want to exit? To confirm, type 'Y', if not, type 'N': ")
            try:
                if quitCheck == "Y":
                    quitCheckedBool = True
                    endSim = True
                elif quitCheck == "N":
                    quitCheckedBool = True
                    endSim = False
                ...
                quitCheckedBool = False
                print(f"An error occurred ({Exception}). Please enter a valid input [Y/N].")
        # TASK 1 CHANGE end
